...
export const EditDay = ({ opened, setOpened, pushDay, courseId, day }) => {
  ...
  const [video, setVideo] = useState(day?.video?.toString() || "");
  const [name, setName] = useState(day?.name?.toString() || "");
  ...
  const saveDay = (e) => {
    ...
    axios.put(`/courses/${courseId}/days`, body).then((res) => {
      if (res.status === 200) {
        //     pushDay(res.data);
        //     showNotification({
        //       title: "День добавлен",
